...
// Dynamic import of TS sources via tsx — Astro already has TS runtime helpers.
// Fallback: parse the ts files as simple strings if tsx isn't available.
...
async function main() {
  ...
  try {
    ...
    // tsx not installed; fall back to minimal llms-full.txt without full content.
    console.warn('llms-full.txt: failed to dynamic-import TS i18n files:', e.message);
    const fallback = '# Tenebris\nUkrainian interceptor drone systems.\n';
    writeFileSync(resolve(ROOT, 'public/llms-full.txt'), fallback);
    return;
  }

  const section = (title, lines) => `\n## ${title}\n\n${lines.filter(Boolean).join('\n\n')}\n`;

  const renderLocale = (loc, data, heading) => {
    ...
    return [
      ...
      section(home.advantages.heading, advIds.map((id) => {
  ...
  const content = [
    renderLocale('en', en, 'Tenebris — Ukrainian interceptor drone systems (English)'),
    '\n---\n',
    renderLocale('uk', uk, 'Tenebris — Українські системи дронів-перехоплювачів'),
    '\n---\n',
