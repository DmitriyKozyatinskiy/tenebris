...
Open XML standard (spec: December 2025) for machine-readable AI content licensing. Co-created by Eckart Walther (RSS co-creator) and Doug Leeds (former CEO of Ask.com).

**Backed by:** Reddit, Yahoo, Medium, Associated Press, The Guardian, USA Today, BuzzFeed, Stack Overflow, Vox Media, Fastly. 50+ partners and 1,500+ media outlets.

**Primary use case:** Publishers who want to monetize or restrict AI usage (pay-per-crawl, pay-per-inference). Less relevant for marketing sites that want maximum AI visibility — robots.txt + llms.txt already signal openness.

**Check for:** RSL implementation, correct XML structure (`<legal type="...">` attributes, not child elements), and valid `<standard>` license URL (e.g., CC BY 4.0: `https://creativecommons.org/licenses/by/4.0/`).
